...
Get your `publishable_key` on your Stripe dashboard:

https://dashboard.stripe.com/test/apikeys

For the `secret_key` field you can paste either of the following:

- your Stripe **secret key** (`sk_test_…` / `sk_live_…`) from the Stripe dashboard above, or
- a **Restricted API Key** (`rk_test_…` / `rk_live_…`) generated by the Sylius Stripe App's _Settings Page_ after installing 
  the app on your Stripe account. RAKs are recommended because they ship with the minimum scopes the plugin needs.
